...
public class ParseDBF
{
    ...
    public static DataTable ReadDBF(string dbfFile)
    {
        ...
        try
        {
            ...
            {
                if (field.fieldName.ToString().Contains("LIEGENNR") || field.fieldName.ToString().Contains("PLZ") || field.fieldName.ToString() == "ORT" || field.fieldName.ToString().Contains("STRASSE"))
                {
            ...
            for (int counter = 0; counter <= header.numRecords - 1; counter++)
            {
                ...
                foreach (FieldDescriptor field in fields)
                {
                    string inhalt = Encoding.ASCII.GetString(recReader.ReadBytes(field.fieldLen));
                    if (field.fieldName.ToString().Contains("LIEGENNR") || field.fieldName.ToString().Contains("PLZ") || field.fieldName.ToString().Contains("STRASSE") || field.fieldName == "ORT")
                    {
                        ...
                        switch (field.fieldType)
                        {
                            case 'N':  // Number
                                if (IsNumber(inhalt))
                                {
                                    if (inhalt.IndexOf(".") > -1)
                                    {
                                        row[fieldIndex] = decimal.Parse(inhalt);
                                    }
                                    else
                                    {
                                        row[fieldIndex] = int.Parse(inhalt);
                                    }
                                }
                                else
                                {
